...
export default function ReaderPage() {
  ...
  // FIXED-AXIS ORP: This forces the red letter to stay in the exact same spot
  const renderFixedWord = (word: string) => {
    if (!word) return null;
    const midpoint = Math.floor(word.length / 2);
    const partLeft = word.substring(0, midpoint);
    const pivot = word.substring(midpoint, midpoint + 1);
    const partRight = word.substring(midpoint + 1);

    return (
      <div className="flex w-full justify-center items-center font-black italic uppercase tracking-tighter text-white transition-all">
        {/* Left part: Right-aligned to the pivot */}
        <div className="w-1/2 text-right pr-[2px] opacity-80">{partLeft}</div>
        
        {/* Pivot: The Locked Point of Axis */}
        <div className="text-red-600 scale-110 drop-shadow-[0_0_10px_rgba(220,38,38,0.5)]">{pivot}</div>
        
        {/* Right part: Left-aligned to the pivot */}
        <div className="w-1/2 text-left pl-[2px] opacity-80">{partRight}</div>
      </div>
    );
  };
      ...
      {!zenMode && (
        <header className="max-w-7xl mx-auto flex justify-between items-start mb-8 text-white">
          ...
          <div className="flex gap-4">
            ...
            <button onClick={() => setZenMode(!zenMode)} className="px-4 py-2 glass-card text-[10px] font-mono uppercase tracking-widest hover:text-red-500 transition-all">Zen: {zenMode ? "ON" : "OFF"}</button>
        ...
        <section className={`${zenMode ? 'fixed inset-0 flex items-center justify-center' : 'col-span-9'}`}>
          <div className={`glass-card neural-glow aspect-video flex items-center justify-center relative overflow-hidden ${zenMode ? 'w-full h-full rounded-none border-none' : ''}`}>
            ...
            <div className="absolute bottom-0 left-0 w-full h-1 bg-white/5">
                <div className="h-full bg-red-600 transition-all duration-300 shadow-[0_0_15px_rgba(220,38,38,0.6)]" style={{ width: `${stats.progress}%` }} />
            </div>

            {/* The Stationary Point of Axis Container */}
            <div className={`relative w-full transition-all ${zenMode ? 'text-8xl md:text-[14rem]' : 'text-6xl md:text-9xl'}`}>
              {renderFixedWord(text)}
              
              {/* Optional: Visual crosshair to help train the eye */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[2px] h-12 bg-white/5 pointer-events-none" />
            </div>
